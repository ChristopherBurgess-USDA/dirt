...
soil_water_data = (
    pd.read_csv(data_path + "soil_water_content.csv")
    .assign(
...
import_weights = (
    pd.read_csv(data_path + "peptidase/tube_weights.csv")
...
casin_constant = pd.read_csv(data_path + "peptidase/casin_blank.csv").mean().to_dict()
...
def peptidase_file_parse(treatments, inc_time):
    ...
    file_path = (
        data_path + "peptidase/peptidase_" + treatments + "_" + inc_time + ".csv"
    )
    import_data = pd.read_csv(file_path).pipe(pd.melt, id_vars=["sample", "rep_std"])

    std_data = (
        import_data.query('sample == "std"')
    ...
    std_0 = std_data.query("rep_std == 0").value.mean()
    ...
    activity_data = (
        ...
        .agg(pred=("pred", np.mean), absorbance=("value", np.mean))
        .reset_index()
        .pipe(
            pd.merge,
            import_weights,
...
def peptidase_parser(treatment_header):
    ...
    parsed_r2 = pd.DataFrame([std_r2], index=[treatment_header], columns=times)

    parsed_data = (
        pre_data[0]
        .pipe(pd.merge, pre_data[1], how="left", on=["sample", "rep"])
        .pipe(pd.merge, pre_data[2], how="left", on=["sample", "rep"])
    )
...
column_reorder = [
    item1 + item2 for item1 in column_header for item2 in ["0h", "24h", "48h"]
]
...
pep_data = pd.concat(pep_data).rename(columns={"sample": "sample_id"})

std_r2 = pd.concat(std_r2)

pep_data.to_csv("data/enzyme/peptidase_activity.csv", index=False)

# %%

test = pd.read_csv(data_path + "peptidase/peptidase_co_dl_0h.csv").pipe(
    pd.melt, id_vars=["sample", "rep_std"]
)
...
test_data = test.query('sample != "std"').filter(["rep_std", "value"])

test_0 = test_std.query("rep_std == 0").value.mean()
...
test_data = (
    test.query('sample != "std"')
    .assign(pred=lambda x: model.predict(sm.add_constant(x["value"])))
...
chart = alt.Chart(test_std).mark_circle().encode(x="value", y="rep_std")
