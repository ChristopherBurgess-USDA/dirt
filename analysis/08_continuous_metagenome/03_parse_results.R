...
data_path = "data/08_continuous_metagenome/"
...
metacyc_results = tibble(
  ...
  mutate(
    type = "metacyc",
    comparison = str_extract(data_path, pattern = "[:alpha:]+_[:alpha:]+_"),
    comparison = str_sub(comparison, 1, -2),
...
ko_results <- tibble(
  ...
  mutate(
    type = "ko",
    comparison = str_extract(data_path, pattern = "[:alpha:]{2}_co"),
    data = list(read_tsv(paste0(data_path, "/significant_results.tsv")))
  ) %>%
  filter(nrow(data)>0) %>%
  unnest(data)
